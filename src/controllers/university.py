...
class UniversityController:
    ...
    async def get_university(self, university_id):
        university = await self.university_repository.get_university_by_id(university_id)
        return university

    async def create_university(self, data):
    ...
    async def update_university(self, university_id, data):
    ...
    async def delete_university(self, university_id):
        db_university = await self.university_repository.get_university_by_id(university_id)
        if not db_university:
            raise BadRequestException("University with this id not exists")
